...
import 'package:quizo/models/player.dart';
...
class FakeQuizApi extends QuizApi {
  FakeQuizApi({
    Set<int>? timeoutQuestionIds,
    Map<int, double>? speedMultipliers,
  ...
  Future<SessionStart> startRush(GameMode mode) async {
    ...
    return SessionStart(
      sessionId: 1,
      questions: rushModeQuestionsOverride ?? rushQuestions,
    );
  }
  ...
  Future<AnswerResult> submitAnswer({
    ...
    return AnswerResult(
      ...
      rushComplete:
          (endRushOnFirstMiss && !isCorrect) ||
          answered >= rushQuestions.length,
  ...
  Future<ClueReveal> revealClue(int sessionId) async {
    revealClueCallCount++;
    return revealClueResultOverride ??
        const ClueReveal(
          cluesRevealed: 2,
          clueText: 'Clue text',
          cluesTotal: 3,
  ...
  Future<RemoveOneResult> useRemoveOne(int sessionId) async {
    ...
    return useRemoveOneResultOverride ??
        const RemoveOneResult(removedOptionIndex: 1, usesRemaining: 0);
  }
  ...
  Future<SessionSummary> finishSession(int sessionId) async {
    ...
    return SessionSummary(
      ...
      correctCount: submittedQuestionIds
          .where((id) => id.isOdd && !timeoutQuestionIds.contains(id))
          .length,
      ...
      isNewPersonalBest:
          personalBestScoreOverride == null ||
          _score > personalBestScoreOverride!,
  ...
  Future<PlayerProfile> getProfile() async {
    ...
    return PlayerProfile(
      ...
      records: ProfileRecords(
        bestRushScore: _score,
        bestStreak: _bestStreak,
        bestAccuracyPct: 50,
        fastestAvgResponseTimeMs: 1200,
        perfectRushCount: isPerfectRushOverride ? 1 : 0,
      ),
      achievements: const [],
    );
  }
...
void main() {
  ...
  testWidgets(
    'Rush progresses through all 10 questions in server-assigned order and completes',
    (tester) async {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        expect(
          controller.screen,
          AppScreen.feedback,
          reason: 'the feedback screen shows before the next question/results',
        );
        await tester.pump(
          const Duration(milliseconds: 1700),
        ); // past the feedback-screen delay
        expect(controller.screen, i == 9 ? AppScreen.results : AppScreen.game);
      }
      ...
      expect(
        fakeApi.submittedQuestionIds,
        fakeApi.rushQuestions.map((q) => q.id).toList(),
      );
    },
  );

  testWidgets(
    'wrong answers break the streak; score/streak reflect server-graded totals',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'Rush result carries the server-authoritative summary (avg time, duration, personal best)',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
  ...
  test('speedLabelFor: thresholds and untimed questions', () {
    ...
    expect(
      speedLabelFor(1.45, hasTimer: false),
      '',
    ); // no speed read on untimed questions
  });
  ...
  testWidgets(
    'momentum rises more on a fast correct answer than a normal-speed one',
    (tester) async {
      final fastApi = FakeQuizApi(
        speedMultipliers: {1: 1.4},
      ); // >= 1.25 counts as "fast"
      ...
      await tester.pump();
      await normalController.selectAnswer(
        0,
      ); // question id 1: normal-speed correct
      await tester.pump();
  ...
  testWidgets(
    'a timeout drops momentum more sharply than a wrong answer, and resets streak',
    (tester) async {
      ...
      final wrongController = await playThroughDivergencePoint(
        FakeQuizApi(speedMultipliers: {1: 1.4, 3: 1.4}),
      );
      final timeoutController = await playThroughDivergencePoint(
        FakeQuizApi(
          speedMultipliers: {1: 1.4, 3: 1.4},
          timeoutQuestionIds: {4},
        ),
      );
  ...
  testWidgets(
    'streak milestones (3, 5, 7, 10) are flagged for the celebratory feedback state',
    (tester) async {
      ...
      allCorrect.rushQuestions
        ..clear()
        ..addAll(
          List.generate(10, (i) => _untimedQuestion(2 * i + 1)),
        ); // 1,3,5,...,19 — all odd
  ...
  testWidgets(
    'a perfect Rush (no wrong answers) is reflected in the finish summary',
    (tester) async {
  ...
  testWidgets(
    'a new best streak is surfaced separately from a new personal-best score',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..personalBestScoreOverride =
            999999 // this run can't beat the score record
  ...
  testWidgets(
    'repeated Rushes (Play Again) reset momentum and per-Rush achievement flags',
    (tester) async {
      ...
      expect(
        controller.screen,
        AppScreen.results,
      ); // Play Again is only reachable from here
  ...
  testWidgets(
    'finishing a Rush surfaces the server-awarded XP, level-up, and unlocked achievements',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ...
        ..newlyUnlockedOverride = const [
          Achievement(
            key: 'first_rush',
            name: 'First Rush',
            description: 'Complete your first Rush.',
          ),
        ];
      final controller = QuizController(quizApi: fakeApi);
      ...
      expect(controller.newlyUnlockedAchievements.map((a) => a.key), [
        'first_rush',
      ]);
    },
  );

  testWidgets('finishing a Rush refreshes the lifetime profile', (
    tester,
  ) async {
    final fakeApi = FakeQuizApi();
    final controller = QuizController(quizApi: fakeApi);
    ...
    await tester.pump();
    expect(
      fakeApi.getProfileCallCount,
      0,
    ); // selectCategory itself doesn't touch the profile
  ...
  testWidgets('goToProfile switches screens and (re)loads the profile', (
    tester,
  ) async {
    final fakeApi = FakeQuizApi();
    final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'a failed profile load is non-fatal — it just leaves profile unset',
    (tester) async {
  ...
  testWidgets(
    'startDailyRush begins a Rush sourced from the daily-rush endpoint, not a normal category',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'resuming an in-progress Daily Rush starts the local qIndex at the server-reported current_index',
    (tester) async {
  ...
  testWidgets(
    'finishing a Daily Rush surfaces rank and daily-best comparison, and refreshes the daily status',
    (tester) async {
      final fakeApi = FakeQuizApi()
      ...
      expect(
        fakeApi.getDailyRushStatusCallCount,
        greaterThan(initialStatusLoads),
      );
    },
  );

  testWidgets(
    'finishing a normal (non-daily) Rush does not touch the daily status endpoint',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets('a Rush ends as soon as the server reports rushComplete, even with questions left locally '
      '(Streak Rush: one miss ends the run)', (tester) async {
    ...
    expect(
      controller.screen,
      AppScreen.game,
      reason: 'still going after a correct answer',
    );
  ...
  testWidgets(
    ...
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
      expect(fakeApi.getHomeCallCount, 0);
      expect(fakeApi.getDailyRushStatusCallCount, 0);

      controller.goToEvents();
      await tester.pump();

      expect(controller.screen, AppScreen.events);
      expect(
        fakeApi.getHomeCallCount,
        1,
        reason: 'entering Events refreshes missions/streak/active events',
      );
      expect(
        fakeApi.getDailyRushStatusCallCount,
        1,
        reason: 'entering Events also refreshes the Daily Rush tile',
      );
      expect(controller.homeSummary, isNotNull);
      expect(controller.dailyRushStatus, isNotNull);
    },
  );

  testWidgets(
    'goToLeaderboard switches screens and loads the requested period',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'starting a fresh (non-daily) Rush resets any leftover Daily Rush result state',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..isDailyRushOverride = true
        ..dailyRankOverride = 2
      ...
      await controller.selectAnswer(
        0,
      ); // just enough to reach /finish eventually isn't needed here
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 1700));
      ...
      controller.goHome();
      await tester.pump();
      fakeApi.isDailyRushOverride =
          false; // the next Rush this fake grades is a normal one
      await controller.selectCategory(_testCategory);
      await tester.pump();
  ...
  testWidgets(
    'revealClue asks the server and advances clueCount for a progressive question',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..revealClueResultOverride = const ClueReveal(
          cluesRevealed: 2,
          clueText: 'Clue 2',
          cluesTotal: 3,
        );
  ...
  testWidgets('revealClue is a no-op for a non-progressive question', (
    tester,
  ) async {
    final fakeApi =
        FakeQuizApi(); // default question is plain text, has no clues
    final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'useRemoveOne marks an option removed and adopts the server-reported remaining count',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..useRemoveOneResultOverride = const RemoveOneResult(
          removedOptionIndex: 2,
          usesRemaining: 0,
        );
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'useRemoveOne is a no-op once already used on the current question',
    (tester) async {
      final fakeApi = FakeQuizApi();
      final controller = QuizController(quizApi: fakeApi);
      ...
      await tester.pump();
      await controller
          .useRemoveOne(); // swallowed by the removedOptionIndex guard
      await tester.pump();
  ...
  testWidgets(
    'a Double Down offer surfaces before the next question, and a risky choice/outcome flow through',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..doubleDownOfferOverride = const DoubleDownOffer(questionId: 2);
      final controller = QuizController(quizApi: fakeApi);
  ...
  testWidgets(
    'answering a question with a real next-question audio/image URL does not throw '
    '(prefetch has no real network/platform channel in a widget test, must degrade silently)',
    (tester) async {
      final fakeApi = FakeQuizApi();
      fakeApi.rushQuestions[1] = Question(
        id: fakeApi.rushQuestions[1].id,
        type: QuestionType.text,
        label: 'Label',
        prompt: 'Prompt',
        options: const ['A', 'B', 'C', 'D'],
  ...
  testWidgets(
    'Home summary (streak/missions/events) refreshes after finishing a Rush',
    (tester) async {
      final fakeApi = FakeQuizApi()
      ...
      expect(
        fakeApi.getHomeCallCount,
        0,
        reason: 'selectCategory alone does not touch home — that only happens on boot/login and after finishing',
      );
      ...
      expect(
        fakeApi.getHomeCallCount,
        1,
        reason: 'finishing a Rush refreshes the home summary',
      );
  ...
  testWidgets(
    'newly completed missions and an extended daily streak surface on the results screen state',
    (tester) async {
      final fakeApi = FakeQuizApi()
      ...
      await tester.pump();
      expect(
        controller.dailyStreakCurrent,
        0,
      ); // nothing reported yet — fresh Rush
      ...
      expect(controller.newlyCompletedMissions.map((m) => m.key), [
        "complete_daily_rush",
      ]);
      expect(controller.lastXpMultiplierApplied, 2.0);
      expect(controller.dailyStreakCurrent, 4);
      expect(
        controller.dailyStreakJustExtended,
        true,
        reason: '4 > the previous known value of 0',
      );
    },
  );

  testWidgets(
    'Play Again resets the per-Rush mission/streak-extended flags but keeps the known streak count',
    (tester) async {
      final fakeApi = FakeQuizApi()
        ..newlyCompletedMissionsOverride = const [
          CompletedMission(
      ...
      expect(
        controller.dailyStreakCurrent,
        3,
        reason: 'the known streak count itself is not a per-Rush flag',
      );
    },
  );

  testWidgets(
    'Home screen renders without layout overflow on a narrow phone width, fully loaded',
    (tester) async {
      addTearDown(tester.view.reset);
      tester.view.physicalSize = const Size(
        360,
        800,
      ); // a common narrow Android width — the tightest case
      ...
      final controller = QuizController(quizApi: FakeQuizApi())
        ..player = const Player(
          ...
          displayName: 'A Fairly Long Display Name',
        )
        ..profile = const PlayerProfile(
          ...
          records: ProfileRecords(
            ...
            bestStreak: 9,
            bestAccuracyPct: 90,
            fastestAvgResponseTimeMs: 900,
            perfectRushCount: 2,
          ),
          achievements: [],
        )
        ..dailyRushStatus = const DailyRushStatus(
          date: '2026-08-24',
          secondsUntilReset: 3600,
          available: true,
          completed: false,
        ...
        ..homeSummary = const HomeSummary(
          dailyStreakCurrent: 7,
          dailyStreakLongest: 10,
          missions: [
            Mission(
              key: 'complete_1_rush',
              name: 'Warm Up',
          ...
          activeEvents: [
            GameEvent(
              ...
              description: 'Every Rush earns double XP.',
              type: 'xp_multiplier',
              multiplier: 2,
              endsAt: null,
            ),
          ],
          leaderboardPosition: LeaderboardPosition(rank: 3, total: 250),
        );
  ...
  testWidgets(
    'Events screen (Daily Rush/streak/missions/stats) renders without layout overflow on a narrow phone width',
    (tester) async {
      addTearDown(tester.view.reset);
      ...
      await tester.pumpWidget(
        MaterialApp(home: EventsScreen(controller: controller)),
      );
      await tester.pump();
  ...
  testWidgets('Tapping a Rush mode card starts that mode via the controller', (
    tester,
  ) async {
    final fakeApi = FakeQuizApi();
    final controller = QuizController(quizApi: fakeApi);

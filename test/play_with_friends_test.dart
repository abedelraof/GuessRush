...
List<Question> _matchQuestions() =>
    List.generate(10, (i) => _untimedQuestion(i + 1));
...
void main() {
  ...
  QuizController buildController(
    _FakeMatchApi api,
    StreamController<MatchEvent> events,
  ) {
    return QuizController(quizApi: api, matchEvents: events.stream);
  }

  testWidgets('goToPlayWithFriends navigates to the mode-select screen', (
    tester,
  ) async {
  ...
  testWidgets(
    'startRandomQueue with no one waiting shows the searching state',
    (tester) async {
      final api = _FakeMatchApi()
        ..joinQueueResult = const QueueJoinResult(status: 'waiting');
      final events = StreamController<MatchEvent>.broadcast();
      addTearDown(events.close);
      final controller = buildController(api, events);
  ...
  testWidgets(
    'startRandomQueue matched immediately begins the match without waiting for a socket event',
    (tester) async {
      final api = _FakeMatchApi()
  ...
  testWidgets(
    'a match:paired push begins the match for the player who was already waiting',
    (tester) async {
      final api = _FakeMatchApi();
      final events = StreamController<MatchEvent>.broadcast();
      addTearDown(events.close);
  ...
  testWidgets(
    'opponent_progress events update opponentQuestionIndex without touching own state',
    (tester) async {
      final api = _FakeMatchApi()
        ..joinQueueResult = QueueJoinResult(
          status: 'matched',
      ...
      expect(
        controller.qIndex,
        0,
        reason: "opponent's progress never touches this player's own qIndex",
      );
    },
  );

  testWidgets(
    'a match:result event for the active match populates matchResult',
    (tester) async {
      final api = _FakeMatchApi()
        ..joinQueueResult = QueueJoinResult(
          status: 'matched',
  ...
  testWidgets('a match:result event for a different match id is ignored', (
    tester,
  ) async {
    final api = _FakeMatchApi()
      ..joinQueueResult = QueueJoinResult(
  ...
  testWidgets(
    'a queue:timeout event while searching surfaces an error and returns to mode-select',
    (tester) async {
      final api = _FakeMatchApi();
      final events = StreamController<MatchEvent>.broadcast();
      addTearDown(events.close);
  ...
  testWidgets(
    'a match:cancelled event while waiting surfaces an error and returns to mode-select',
    (tester) async {
      final api = _FakeMatchApi();
      final events = StreamController<MatchEvent>.broadcast();
      addTearDown(events.close);
  ...
  testWidgets('cancelQueue leaves the queue and returns to mode-select', (
    tester,
  ) async {
    final api = _FakeMatchApi();
    final events = StreamController<MatchEvent>.broadcast();
  ...
  testWidgets(
    'joinFriendMatch failure surfaces the error without changing screens',
    (tester) async {
      final api = _FakeMatchApi()
        ..joinFriendMatchError = ApiException(
  ...
  testWidgets(
    'Play With Friends screen renders both mode cards without layout overflow',
    (tester) async {
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: PlayWithFriendsScreen(controller: controller)),
        ),
      );
      await tester.pump();
  ...
  testWidgets('tapping the random-opponent card starts the queue', (
    tester,
  ) async {
    final api = _FakeMatchApi();
    final controller = buildController(
  ...
  testWidgets(
    'Match waiting screen renders the searching state without layout overflow',
    (tester) async {
      addTearDown(tester.view.reset);
      tester.view.physicalSize = const Size(360, 800);
      tester.view.devicePixelRatio = 1.0;
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(body: MatchWaitingScreen(controller: controller)),
        ),
      );
      await tester.pump();
  ...
  testWidgets(
    'Match waiting screen renders the friend code-entry state without layout overflow',
    (tester) async {
      addTearDown(tester.view.reset);
      tester.view.physicalSize = const Size(360, 800);
      tester.view.devicePixelRatio = 1.0;
  ...
  testWidgets('Results screen with a match banner AND a full burst of first-Rush badges renders '
      'without layout overflow on a narrow phone width', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: ResultsScreen(controller: controller)),
      ),
    );
    await tester.pump();
    ...
    expect(
      ...
      isNull,
      reason: 'no RenderFlex overflow at 360dp width',
    );
